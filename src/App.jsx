/**
    ...
    // Use EventSource (GET) for robust SSE handling in browsers
    const q = new URLSearchParams({ topic, max_revisions: String(maxRev), score_threshold: String(scoreThr) });
    const es = new EventSource(`${API_BASE}/api/research-stream?${q.toString()}`);
    ...
    const onNodeStart = (e) => {
      try {
        ...
        setNode(n, "running");
        console.debug('EventSource node_start', payload);
    ...
    const onNodeDone = (e) => {
      try {
        ...
        console.debug('EventSource node_done', payload);
    ...
    const onPipelineEnd = (e) => {
      try {
        ...
        console.debug('EventSource pipeline_end', payload);
      } catch (err) { console.error(err); }
      es.close();
      setRunning(false);
    };
    ...
    const onError = (e) => {
      addLog("SSE connection error", "red");
    ...
    es.onopen = () => console.debug('EventSource opened');

    // Safety: close connection after a timeout or when user navigates away
    const cleanup = () => {
      es.removeEventListener('node_start', onNodeStart);
      es.removeEventListener('node_done', onNodeDone);
      es.removeEventListener('pipeline_end', onPipelineEnd);
      es.removeEventListener('error', onError);
      try { es.close(); } catch (e) {}
      setRunning(false);
    };

    // attach to window unload to avoid leaked connections
  ...
  async function runPipeline() {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/research`, {
        ...
        body: JSON.stringify({ topic, max_revisions: maxRev, score_threshold: scoreThr }),
      });

      console.debug("Fetch response headers:", Array.from(res.headers.entries()));
      if (!res.ok) {
      ...
      while (true) {
        ...
        // SSE frames are separated by a double newline; handle CRLF or LF
        ...
        for (const frame of frames) {
          ...
          console.debug("SSE frame parsed", { eventType, dataLine });
